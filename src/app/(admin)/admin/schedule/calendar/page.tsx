...
export default async function AdminScheduleCalendar({
  ...
  const selected = appt && view === "day" ? dayAppointments.find((a) => a.id === appt) : undefined;
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">All doctors&apos; schedule</p>
        </div>
      ...
      <div className="overflow-hidden rounded-lg border">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b px-4 py-3">
          <div className="flex items-center gap-2">
            <Link href={buildUrl(view, prev, doctorId)} className={cn(buttonVariants({ variant: "outline", size: "sm" }))}>
              ‹
            </Link>
            <Link href={buildUrl(view, next, doctorId)} className={cn(buttonVariants({ variant: "outline", size: "sm" }))}>
              ›
            </Link>
            <Link href={buildUrl(view, today, doctorId)} className={cn(buttonVariants({ variant: "outline", size: "sm" }))}>
              Today
            </Link>
          </div>
              ...
              <div className="relative">
                ...
                {dayAppointments.map((a) => {
                  ...
                  return (
                    <Link
                      key={a.id}
                      href={buildUrl("day", day, doctorId, a.id)}
                      className={cn(
                    ...
                    >
                      ...
                      <span className={cn("truncate", isSelected ? "text-primary-foreground/80" : "text-blue-900/70")}>
                        {apptDate.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })} · Dr. {a.doctor.user.name}
                      </span>
                    </Link>
                  );
                })}
            ...
            <div className="w-[280px] shrink-0 border-l bg-muted/30 px-4 py-4">
              {selected ? (
                <div className="flex flex-col gap-2 rounded-lg bg-background p-4 shadow-sm">
                  ...
                  <p className="text-sm text-muted-foreground">Doctor: Dr. {selected.doctor.user.name}</p>
                  ...
                  <div key={d.toISOString()} className="relative border-l">
                    ...
                    {dayAppts.map((a) => {
                      ...
                      return (
                        <Link
                          key={a.id}
                          href={buildUrl("day", d, doctorId, a.id)}
              ...
              {monthWeeks.map((week, wi) =>
                week.map((cell, di) => {
                  ...
                  return (
                    <Link
                      key={`${wi}-${di}`}
                      href={buildUrl("day", cell.date, doctorId)}
                      className={cn(
