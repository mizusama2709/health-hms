...
export default async function PatientChartPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // The most recent order is the clinically relevant "current state" — an
  // abnormal result from an old, since-superseded order shouldn't keep
  // flashing a permanent warning.
  const alerts: ChartAlert[] = (labOrders[0]?.items ?? [])
    .filter((item): item is typeof item & { flag: "CRITICAL" | "ABNORMAL" } => item.flag === "CRITICAL" || item.flag === "ABNORMAL")
    .sort((a, b) => (a.flag === b.flag ? 0 : a.flag === "CRITICAL" ? -1 : 1))
    .map((item) => ({
                    ...
                    {order.studies.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-3 text-sm">
                        {order.studies.map((s) => (
                          <Link
                            key={s.id}
                            href={`/admin/patients/${patient.id}/imaging/${s.id}`}
                            className="font-medium text-primary hover:underline"
                          >
