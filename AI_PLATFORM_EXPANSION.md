...
**This is a plan, not a build log** for everything except §C4 (PR #5), which has shipped and is merged into `master`. §B1, §C3, and §C9 (LLM client, Doctor Co-Pilot, InZi Assistant) **were built in PR #8, then deliberately removed in PR #13** — see the callout below. They're back to unbuilt-plan status in this document. Everything not yet built can be demoed against seed data first, then pointed at real credentials later, once §A's vendor decisions are made.
...
**PR #13 removed Doctor Co-Pilot and InZi Assistant entirely** (`lib/ai/client.ts`, `lib/copilot.ts`, `src/components/inzi/`, both `/admin/assistant` and `/doctor/assistant` routes, the `@anthropic-ai/sdk` dependency, and their nav entries) — verified clean, no dangling references anywhere in source or `package.json`. Reason stated in the PR: `ANTHROPIC_API_KEY` was never actually configured, locally or on Vercel, so the feature was permanently non-functional dead code from the moment it shipped. **If §B1/§C3/§C9 get rebuilt from this plan in the future, get the API key configured and verified working *before* merging** — that's the concrete lesson from this round-trip, not just "build it and hope the key shows up later." The same PR also reworked `/doctor/calendar` into a macOS Calendar–style day view, unrelated to the AI removal.
...
### B1. `lib/ai/client.ts` — single LLM entry point — ⚠️ built in PR #8, removed in PR #13
Was built as one wrapped Anthropic SDK client (`claude-sonnet-4-5`), with `isAiConfigured()`/`getClient()` throwing a clear catchable error instead of a raw SDK exception when the key was missing. Only `generateConsultSummary` (for C3) and `answerClinicalQuery` (for C9) were ever implemented — `classifyWhatsAppIntent` (C1), `extractLabValues` (C6), `extractPaymentScreenshot` (C7) were never built. **Deleted in PR #13** because `ANTHROPIC_API_KEY` was never configured anywhere, so nothing built on top of it could ever produce a real answer. If this gets rebuilt, get the key configured and verified working first — don't ship the client again on the assumption the key will show up later.
...
### C3. Doctor Co-Pilot (§3) — ⚠️ shipped in PR #8, removed in PR #13
Was built as `lib/copilot.ts`'s `generatePreConsultSummary(appointmentId, tenantId)`, gathering `VisitRecord` history, `LabOrder` results, pending `Prescription`s, and `KnowledgeBaseDocument` titles, surfaced as a "Co-Pilot summary →" link on every `/doctor` appointment row. Deleted in PR #13 along with B1 (`ANTHROPIC_API_KEY` never configured, so it never produced a real summary). If rebuilt: the guideline-grounding gap from the original build still applies — `KnowledgeBaseDocument` only stores a `fileUrl` link, not extracted text, so real grounding needs a fetch+parse step that was never built either.
...
### C9. InZi AI Assistant (§9) — ⚠️ shipped in PR #8, removed in PR #13
Was built as `src/components/inzi/inzi-chat.tsx` — a client component (the first genuinely stateful client-side UI in an otherwise server-action-only app, as planned), serving two scoped routes: `/admin/assistant` (staff) and `/doctor/assistant` (clinical). Deleted in PR #13 along with B1 for the same reason (never had a working API key). Never had knowledge-base retrieval or conversation persistence even before removal — both were open gaps in the original build. Both routes now 404, confirmed.
...
## E. Suggested build order (revised after PR #13's removal — B1/C3/C9 are back at the front of the queue, but gated on the API-key lesson below)

1. ~~**C4** (Vitals capture)~~ — ✅ shipped (PR #5), plus the unplanned Queue live-tracker rebuild (PR #6) that now hosts it. This one's durable — no vendor dependency, nothing to remove.
2. **B1 + C3 + C9, take two** (LLM client, Doctor Co-Pilot, InZi Assistant) — built once already (PR #8), removed once already (PR #13) for shipping without a working `ANTHROPIC_API_KEY`. **Before writing any code this time: confirm the key is set in `.env` and on Vercel, and that a real `messages.create` call succeeds, before building anything on top of it.** The implementation approach from PR #8 (single wrapped client, role-scoped chat component, read-only Co-Pilot summary) was sound — the failure was sequencing, not architecture.
3. **(Optional, anytime) F2** (Rx Templates ✅ shipped PR #12, Quick Phrases, AI service creator) — no AI/WhatsApp dependency for the remaining two, small and independent; good filler work between the AI-dependent phases below.
...
7. **C6** (Lab parsing/alerts) — needs vision extraction; **C5** (interaction checking) waits on §A.3's dataset decision.
