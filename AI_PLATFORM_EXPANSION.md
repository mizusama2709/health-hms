...
**This is a plan, not a build log.** Nothing in this document has been implemented yet. Three items below require you to pick and pay for a vendor before any related code can run against something real (§A). Everything else can be built and demoed against seed data first, then pointed at real credentials later.
...
**Effects on other features:** Replaces the fixed-slot logic entirely — Queue and Reports read from `Appointment`/`PatientJourneyEvent` unchanged, so nothing downstream breaks, they just start seeing appointments booked with real slot logic instead of the demo heuristic. `Inbox` UI (`/admin/inbox`) needs no changes — it already just lists `WhatsAppMessage` rows regardless of how they got created.
...
### C4. Check-in & Vitals Capture (§4)
**Changes:** New `Vitals` model (`patientId`, `appointmentId?`, `bp`, `glucose`, `weight`, `spo2`, `recordedAt`, `source: "IN_CLINIC" | "WHATSAPP"`). New `lib/vitals.ts`: `recordVitals`, `listVitalsForPatient`. `/admin/queue` gains a "Record vitals" action per row that, on submit, also fires the existing `recordJourneyEvent(..., "VITALS_TAKEN")` — **this is the one line that finally makes today's decorative `VITALS_TAKEN` label real**. WhatsApp-based between-visit capture reuses B3's conversation state with a `"AWAITING_VITALS"` state and B1's classifier extracting structured numbers from free text.
**Effects on other features:** `/admin/reports`' Self-Efficacy tab already computes booked→vitals→OPD gaps from `PatientJourneyEvent` — once `VITALS_TAKEN` is actually recorded, those numbers go from "always the fallback/never present" to real, so expect the Self-Efficacy averages to change once this ships (worth a heads-up if anyone's already looking at those numbers as a baseline). Patient chart page (`/admin/patients/[id]`) gets a new vitals trend section.
...
1. **B1 + B2** (LLM client, real WhatsApp client) — foundation, nothing above is real without it. Requires your Anthropic key + WhatsApp provider choice (§A.1, §A.2).
2. **C4** (Vitals capture) — no AI dependency, schema-light, ships value immediately and makes an existing dead journey-step real.
